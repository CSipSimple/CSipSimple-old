...
public class UAStateReceiver extends Callback {
    ...
    public void on_incoming_call(final int acc_id, final int callId, SWIGTYPE_p_pjsip_rx_data rdata) {
        ...
        Thread t = new Thread() {
            ...
            public void run() {
                ...
                /*
                 * SipCallSession existingOngoingCall = getActiveCallInProgress(); if(existingOngoingCall != null) {
                 * if(existingOngoingCall.getCallState() == SipCallSession.InvState.CONFIRMED) { Log.e(THIS_FILE,
                 * "For now we do not support two call at the same time !!!"); //If there is an ongoing call... For now decline TODO : should here
                 * manage multiple calls //Send busy here pjsua.call_hangup(callId, 486, null, null); unlockCpu(); return; } }
                 */
    ...
    public void on_pager(int call_id, pj_str_t from, pj_str_t to, pj_str_t contact, pj_str_t mime_type, pj_str_t body) {
        ...
        SipMessage msg = new SipMessage(displayName, sFrom, to.getPtr(), contactString, body.getPtr(), mime_type.getPtr(), date, SipMessage.MESSAGE_TYPE_INBOX);
        ...
        intent.putExtra(SipMessage.DISPLAY_NAME, msg.getDisplayName());
    ...
    public void on_pager_status(int call_id, pj_str_t to, pj_str_t body, pjsip_status_code status, pj_str_t reason) {
        ...
        int messageType = (status.equals(pjsip_status_code.PJSIP_SC_OK) || status.equals(pjsip_status_code.PJSIP_SC_ACCEPTED)) ? SipMessage.MESSAGE_TYPE_SENT
        ...
        Thread t = new Thread() {
            ...
            public void run() {
                ...
                if (callInfo.getMediaStatus() == SipCallSession.MediaState.ACTIVE) {
                    ...
                    // Auto record
                    if (recordedCall == INVALID_RECORD && pjService.prefsWrapper.getPreferenceBooleanValue(SipConfigManager.AUTO_RECORD_CALLS)) {
    ...
    public int on_setup_audio(int clockRate) {
        if (pjService != null) { return pjService.setAudioInCall(clockRate); }
        return -1;
    }
    ...
    public SipCallSession getActiveCallInProgress() {
        ...
        for (Integer i : callsList.keySet()) {
            ...
            if (callInfo.isActive()) { return callInfo; }
        }
        return null;
    }
    ...
    /**
     * Broadcast the Headset button press event internally if there is any call in progress.
     */
    public boolean handleHeadsetButton() {
        ...
        if (callInfo != null) {
            ...
            if (callInfo.isIncoming() && (state == SipCallSession.InvState.INCOMING || state == SipCallSession.InvState.EARLY)) {
                ...
                return true;
            } else if (state == SipCallSession.InvState.INCOMING || state == SipCallSession.InvState.EARLY
                    || state == SipCallSession.InvState.CALLING || state == SipCallSession.InvState.CONFIRMED
    ...
    public void startRecording(int callId) {
        ...
        if (recordedCall == INVALID_RECORD) {
            ...
            if (callInfo == null || callInfo.getMediaStatus() != SipCallSession.MediaState.ACTIVE) { return; }
    ...
    public boolean canRecord(int callId) {
        if (recordedCall == INVALID_RECORD) {
            SipCallSession callInfo = getCallInfo(callId, false);
            if (callInfo == null || callInfo.getMediaStatus() != SipCallSession.MediaState.ACTIVE) { return false; }
            return true;
        }
        return false;
    }
    ...
    private File getRecordFile(String remoteContact) {
        ...
        if (dir != null) {
            Date d = new Date();
            File file = new File(dir.getAbsoluteFile() + File.separator + sanitizeForFile(remoteContact) + "_"
