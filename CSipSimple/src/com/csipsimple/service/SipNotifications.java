...
public class SipNotifications {
    ...
    public void showNotificationForMessage(SipMessage msg) {
        if (!CustomDistribution.supportMessaging()) { return; }
...
//            CharSequence tickerText = buildTickerMessage(context, from, msg.getBody());
          CharSequence tickerText = buildTickerMessage(context, msg.getDisplayName(), msg.getBody());

            if (messageNotification == null) {
                messageNotification = new Notification(SipUri.isPhoneNumber(from) ? R.drawable.stat_notify_sms : android.R.drawable.stat_notify_chat,
            ...
            notificationIntent.putExtra(SipMessage.DISPLAY_NAME, msg.getDisplayName());
            ...
            if (msg.getDisplayName() != null && msg.getDisplayName().length() > 0)
                messageNotification.setLatestEventInfo(context, msg.getDisplayName(), msg.getBody(), contentIntent);
            else
    ...
    public void showNotificationForVoiceMail(SipProfile acc, int numberOfMessages, String voiceMailNumber) {
        if (messageVoicemail == null) {
            messageVoicemail = new Notification(android.R.drawable.stat_notify_voicemail, context.getString(R.string.voice_mail),
