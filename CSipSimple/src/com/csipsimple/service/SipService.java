...
public class SipService extends Service {
    ...
    private final ISipService.Stub binder = new ISipService.Stub() {
         ...
         * 
         * @param callee
         *            remote contact ot call If not well formated we try to add domain name of the default account
         */
        @Override
        ...
        public void sendMessage(String message, String callee, int accountId) throws RemoteException {
            ...
            if (callee.indexOf("<") != -1 && callee.indexOf(">") != -1) {
                displayName = callee.substring(callee.indexOf("<") + 1, callee.indexOf(">"));
            ...
            if (called != null) {
                SipMessage msg = new SipMessage(displayName, SipMessage.SELF, SipUri.getCanonicalSipContact(callee), SipUri.getCanonicalSipContact(called
                        .getCallee()), message, "text/plain", System.currentTimeMillis(), SipMessage.MESSAGE_TYPE_QUEUED);
                msg.setRead(true);
                synchronized (db) {
                    db.open();
        ...
        public SipCallSession[] getCalls() throws RemoteException {
            SipService.this.enforceCallingOrSelfPermission(SipManager.PERMISSION_USE_SIP, null);
            if (pjService != null) { return pjService.getCalls(); }
    ...
    private class ServiceDeviceStateReceiver extends BroadcastReceiver {
        ...
        /*
         * 
         * @Override public void onReceive(Context context, Intent intent) { // // ACTION_CONNECTIVITY_CHANGED // Connectivity change is used to
         * detect changes in the overall // data network status as well as a switch between wifi and mobile // networks. // //
         * ACTION_DATA_STATE_CHANGED // Data state change is used to detect changes in the mobile // network such as a switch of network type (GPRS,
         * EDGE, 3G) // which are not detected by the Connectivity changed broadcast. // Log.d(THIS_FILE, "ServiceDeviceStateReceiver"); String action
         * = intent.getAction(); if(action == null) { return; }
         * 
         * if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) { //|| intent.getAction( ).equals(ACTION_DATA_STATE_CHANGED) Log.d(THIS_FILE,
         * "Connectivity or data state has changed"); // Thread it to be sure to not block the device if registration // take time Thread t = new
         * Thread() {
         * 
         ...
         * dataConnectionChanged();
         * 
         * } }; t.start(); }else if(action.equals(SipManager.ACTION_SIP_ACCOUNT_ACTIVE_CHANGED)) { final long accountId =
         * intent.getLongExtra(SipManager.EXTRA_ACCOUNT_ID, -1); final boolean active = intent.getBooleanExtra(SipManager.EXTRA_ACTIVATE, false);
         * //Should that be threaded? if(accountId != SipProfile.INVALID_ID) { SipProfile account; synchronized (db) { db.open(); account =
         * db.getAccount(accountId); db.close(); } if(account != null) { setAccountRegistration(account, active?1:0); } } }
         * 
         * 
         * }
         */
    ...
    private void registerBroadcasts() {
        ...
        if (phoneConnectivityReceiver == null) {
            ...
            telephonyManager
                    .listen(phoneConnectivityReceiver, PhoneStateListener.LISTEN_DATA_CONNECTION_STATE | PhoneStateListener.LISTEN_CALL_STATE);
        }
    }
    ...
    public SipProfileState getSipProfileState(int accountDbId) {
        ...
        if (pjService != null) { return pjService.getAccountInfo(account); }
        return null;
    }
    ...
    /**
     * Timer that can schedule keep alives to occur even when the device is in sleep. Only used internally in this package.
     */
    class KeepAliveTimer extends BroadcastReceiver {
        ...
        public void start() {
            ...
            if (pendingIntent != null) { throw new RuntimeException("pendingIntent is not null!"); }
